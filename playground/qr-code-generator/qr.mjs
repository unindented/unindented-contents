 ...
 *
 * @see {@link https://github.com/Rich-Harris/headless-qr}
 * @see {@link https://github.com/kazuhikoarase/qrcode-generator}
 */
...
function create_data(version, error_correction_level, data) {
  ...
  if (buffer.get_length_in_bits() > total_data_count * 8) {
    throw "code length overflow. (" + buffer.get_length_in_bits() + ">" + total_data_count * 8 + ")";
  }
  ...
  multiply(e) {
    ...
    for (let i = 0; i < this.get_length(); i += 1) {
      for (let j = 0; j < e.get_length(); j += 1) {
        num[i + j] ^= QRMath.gexp(QRMath.glog(this.get_at(i)) + QRMath.glog(e.get_at(j)));
      }
    }
  ...
  return {
    ...
    get_rs_blocks(version, error_correction_level) {
      ...
      if (typeof rs_block == "undefined") {
        throw "bad rs block @ version:" + version + "/error_correction_level:" + error_correction_level;
      }
